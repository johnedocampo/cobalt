...
void MediaCodecVideoDecoder::WriteInputBuffers(
  ...
  if (input_buffer_written_ == 0) {
    ...
    if (!IsIdentity(color_metadata) && color_metadata_ != color_metadata) {
...
void MediaCodecVideoDecoder::PreallocateNextCodec(
  ...
  SB_LOG(INFO) << "Pre-allocating next MediaCodec on dummy surface in "
                  "background for warm swap.";

  // Pass a null surface to CreateForVideo so MediaCodecBridge Java creates a
  // dummy Surface.
  jni_zero::ScopedJavaLocalRef<jobject> j_dummy_surface;

  std::optional<Size> max_frame_size = max_video_size_;
  if (max_frame_size.has_value() &&
      IsFrameSizeExceedingCapabilities(video_stream_info.frame_size,
                                       max_frame_size.value())) {
    max_frame_size = video_stream_info.frame_size;
  }
  ...
  if (result) {
    ...
    SB_LOG(INFO) << "Successfully pre-allocated next_media_decoder_ on dummy "
                    "surface for warm swap.";
  } else {
  ...
  if (next_media_decoder_) {
    ...
    switch (output_mode_) {
      ...
      case kSbPlayerOutputModeDecodeToTexture: {
        std::lock_guard lock(decode_target_mutex_);
        if (decode_target_) {
          j_output_surface = jni_zero::ScopedJavaLocalRef<jobject>(
              env, decode_target_->surface());
        }
      } break;
      default:
        break;
    ...
    if (j_output_surface && next_decoder->SetOutputSurface(j_output_surface)) {
      ...
      if (!result) {
        ReportError(kSbPlayerErrorDecode,
                    "Failed fallback codec reinitialization.");
      }
    }
  } else {
    SB_LOG(WARNING) << "next_media_decoder_ not available. Falling back to "
                       "single-decoder re-creation.";
    TeardownCodec();
  }
